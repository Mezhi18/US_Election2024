...
preddata <- data %>%
  filter(numeric_grade >1) %>%
  select(pollster, sample_size, state, party, answer, candidate_id, candidate_name, pct, start_date, end_date, population, pollscore, numeric_grade) %>%
...
print(modelsummary(bayesian_model))
summary(bayesian_model)
```
...
spline_model_pred <- stan_glm(
  pct ~ ns(end_date_num, df = 5) + state + pollster,
  data = harrisdata,
  family = gaussian(),
  prior = normal(0, 5),
...
# Convert `pollster` to a factor with defined levels if it's not already
harrisdata$pollster <- factor(harrisdata$pollster, levels = unique(harrisdata$pollster))

# Define the new data grid for prediction, ensuring each factor has the correct levels
new_data <- data.frame(
  end_date_num = seq(
