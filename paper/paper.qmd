...
preddata <- data %>%
 # filter(pollster_id == 1424 & !is.na(state) & state != "") %>%
...
harrisdata <- preddata %>%
  filter(state %in% c("Georgia", "Arizona", "Nevada", "Michigan", "North Carolina", "Pennsylvania", "")) %>%
...
# Plot the filtered and transformed data
...
## MODEL 1, LINEAR MODEL WITH PREDICTOR END_DATE

model_date <- lm(pct ~ end_date, data = harrisdata)
...
ggplot(harrisdata, aes(x = end_date)) +
...
## MODEL 2, BAYESIAN MODEL WITH PREDICTOR END_DATE
...
ggplot(harrisdata, aes(x = end_date_num, y = pct)) +
  geom_point() +
  geom_line(
    data = pred_summary,
